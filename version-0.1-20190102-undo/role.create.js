...
var createCreeps = {
    run: function() {
        ...
        var repa = _.filter(Game.creeps, (creep) => creep.memory.role == 'repairer');
        if(Game.spawns['LOSKI_1'].energy < 250 || Game.spawns['LOSKI_1'].spawning){
            return ;
        }
        if(harv.length<3){
        ...
        else if(repa.length<3){
            let newName = 'Repairer'+ Game.time;
            console.log('Repairer:' + newName);
            Game.spawns['LOSKI_1'].spawnCreep([WORK,CARRY,MOVE,MOVE], newName,
